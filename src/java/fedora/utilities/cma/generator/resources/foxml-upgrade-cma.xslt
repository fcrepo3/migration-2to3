...
<!--
	...
	be used to define an explicit created data if that behaviour is undesirable.
	...
	the FedoraObject-3.0 model.  Future fedora versions may not.
...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/foxml:digitalObject">
		<xsl:copy>
			...
			<xsl:choose>
				...
				<!-- If the object is a basic fedora object, has no assigned model, and 
					casic model declarations are implicit, then there is no need to create a RELS-EXT -->
				<xsl:when
	...
	<xsl:template
		match="//foxml:datastream[@ID='RELS-EXT']/foxml:datastreamVersion/@MIMETYPE">
		<xsl:attribute name="MIMETYPE">application/rdf+xml</xsl:attribute>
	</xsl:template>

	<!-- Update RELS-INT MIME-type to application/rdf+xml -->
	<xsl:template
		match="//foxml:datastream[@ID='RELS-INT']/foxml:datastreamVersion/@MIMETYPE">
		<xsl:attribute name="MIMETYPE">application/rdf+xml</xsl:attribute>
	</xsl:template>

	<!-- Add the CMA contract relationship to current RELS-EXT, if it exists -->
	<xsl:template
	...
	<xsl:template name="addCmodelRel">
		<xsl:copy>
			...
			<xsl:for-each select="node()">
				<xsl:choose>
					<xsl:when
						test="name(self::node()) = 'rdf:Description'"
						xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
						<xsl:copy>
							<xsl:apply-templates select="@*" />
	...
	<xsl:template name="printCModelRels">
		...
		<xsl:if test="$explicitBasicModel != ''">
			<xsl:element name="fedora-model:hasModel"
				...
				<xsl:attribute name="rdf:resource">
                                    <xsl:value-of
						select="'info:fedora/fedora-system:FedoraObject-3.0'" />
                                </xsl:attribute>
			</xsl:element>
		</xsl:if>
	</xsl:template>
